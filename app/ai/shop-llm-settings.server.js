...
export async function getShopLlmSettings(shop) {
  try {
    ...
    return {
      ...
      maxHistoryMessages: typeof row.maxHistoryMessages === "number" ? row.maxHistoryMessages : null,
      hasAnthropicApiKey,
      anthropicApiKey,
      anthropicApiKeyHint:
 ...
 * @param {number} params.maxHistoryMessages
...
export async function upsertShopLlmSettings({
  ...
  const maxHistory = typeof maxHistoryMessages === "number" && maxHistoryMessages > 0 ? maxHistoryMessages : null;
